...
def _run_cvx(b_vector, A, reg_tol, reg_lambda, pnorm, **kwargs_solver):
    """
...
def complex_cvx(A, y, reg_tol, reg_lambda, reg_norm):
    """
...
def solve_ip(method, A, y, opts=dict()):
    """
    ...
    b_split_realim = (np.sum(np.imag(A)) == 0)
    # if b_split_realim: print("\nsolve_ip: ¡split Real/Imag!")
    # else:              print('\nsolve_ip: complex')
    ...
    if "lasso_lars" in method:
        ...
        if b_split_realim: # separate real and imaginary fields 
            Aci = np.real(A)
            yci = complex_interlace(y,1)
        else:   # combined real and imaginary components wrt sparsity
